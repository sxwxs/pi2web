...
export class MockBackend implements AgentBackend {
 ...
 async getSession(){return {sessionId:this.state.sessionId,entries:[],tree:[],stats:{totalMessages:this.messages.length},contextUsage:{tokens:null,contextWindow:0,percent:null}}}
...
export class AgentManager {
 ...
 private attach(id:string,record:AgentRecord,backend:AgentBackend){const entry={record,backend,events:[] as {id:string,sequence:number,timestamp:number,event:AgentEvent}[],nextSequence:1};backend.subscribe(event=>{record.status=event.type==='agent_start'||event.type==='auto_retry_start'||(event.type==='agent_end'&&event.willRetry===true)?'streaming':event.type==='agent_end'||event.type==='agent_settled'?'idle':record.status;record.lastActiveAt=new Date().toISOString();const item={id:randomUUID(),sequence:entry.nextSequence++,timestamp:Math.floor(Date.now()/1000),event};entry.events.push(item);if(entry.events.length>1000)entry.events.shift();for(const listener of this.listeners.get(id)??[])listener(item);for(const listener of this.globalListeners)listener(id,item)});this.agents.set(id,entry)}
