...
export class MockBackend implements AgentBackend {
 ...
 async compact(){return {summary:'mock'}} async setModel(_provider:string,_modelId:string){} async setThinkingLevel(_level:string){} async navigate(_entryId:string){return {cancelled:false}} async fork(_entryId:string){return undefined} async extensionResponse(_requestId:string,_value:unknown){}
...
export class AgentManager {
 ...
 async setThinkingLevel(id:string,level:string){return this.get(id).backend.setThinkingLevel(level)} async navigate(id:string,entryId:string){return this.get(id).backend.navigate(entryId)}
