...
type TerminalEntry={record:TerminalRecord,process:PtyProcess,emitter:EventEmitter,buffer:string};
...
export class TerminalManager {
  ...
  constructor(private workspaces:WorkspaceStore,private factory:TerminalFactory=defaultFactory,private maxRunning=8,private maxBuffer=512*1024){}
  ...
  async create(workspaceId:string,relativeCwd='.',cols=80,rows=24){
    ...
    process.onData(data=>{entry.buffer=(entry.buffer+data).slice(-this.maxBuffer);entry.record.lastActiveAt=new Date().toISOString();entry.emitter.emit('event',{type:'output',data} satisfies TerminalEvent)});
    process.onExit(event=>{entry.record.status='exited';entry.record.exitCode=event.exitCode;entry.record.lastActiveAt=new Date().toISOString();entry.emitter.emit('event',{type:'exit',exitCode:event.exitCode} satisfies TerminalEvent)});
    return {...record};
  }
  ...
  close(id:string){const entry=this.get(id);this.terminals.delete(id);if(entry.record.status==='running'){entry.emitter.emit('event',{type:'exit',exitCode:-1} satisfies TerminalEvent);try{entry.process.kill()}catch{}}entry.emitter.removeAllListeners()}
