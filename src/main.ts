...
if(voiceBaseUrl){
  ...
  const config:VoiceConfig={speechBaseUrl:voiceBaseUrl,sttModel:value('--voice-stt-model','').trim()||undefined,ttsModel:required('--voice-tts-model'),ttsVoice:required('--voice-tts-voice'),speechApiKey:secret('--voice-api-key-env'),summaryBaseUrl:required('--voice-summary-base-url'),summaryModel:required('--voice-summary-model'),summaryApiKey:secret('--voice-summary-api-key-env'),language:value('--voice-language','zh-CN')};
