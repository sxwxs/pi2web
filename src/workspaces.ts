...
export class WorkspaceStore {
 ...
 async tree(id:string,relative='.') {const ws=this.get(id);if(!ws)throw Object.assign(new Error('Workspace not found'),{code:'WORKSPACE_NOT_FOUND'});const dir=await this.resolve(ws,relative);if(!(await stat(dir)).isDirectory())throw new Error('Not a directory');const entries=await readdir(dir,{withFileTypes:true});return Promise.all(entries.filter(e=>!e.name.startsWith('.')).sort((a,b)=>Number(b.isDirectory())-Number(a.isDirectory())||a.name.localeCompare(b.name)).map(async e=>{const p=path.join(dir,e.name);const s=await lstat(p);return {name:e.name,type:e.isDirectory()?'directory':'file',size:s.size,modifiedAt:s.mtime.toISOString()}}))}
